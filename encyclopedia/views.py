...
def search(request):
    ...
    if q in util.list_entries(): 
        # note that the match succeeds only if the match is case-sensitive exact
        return redirect("entry", title=q)
    return render(request, "encyclopedia/search.html", {"entries": util.search(q.lower()), "q": q})
...
def create(request):
    if request.method == "POST":
        ...
        if title == "":
            return render(request, "encyclopedia/add.html", {"message": "Can't create an entry with empty title.", "title": title, "content": content})
        if content == "":
            return render(request, "encyclopedia/add.html", {"message": "Can't create an entry with empty contents.", "title": title, "content": content})
